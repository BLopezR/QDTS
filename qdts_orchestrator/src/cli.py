...
logging.basicConfig(level=logging.INFO, filename='/tmp/qdts_orchestrator.log', format=log_format, filemode='a')
...
@click.group()
def cli():
    """Command-line interface to execute the qdts-orchestrator"""
    pass


# Command to start the qkd-orchestator
# This install the necessary packages on the qdts nodes, and execute simulaqron
@cli.command()
...
def start(config_file, inv_file, osm_hostname, vim_account, ssh_key):
    ...
    logger.info("Installation of the qdts_node requirements in every node")
    ...
    # If OSM is not specified, execute the qdts_node installation function
    ...
    # Install the qdts_node requirements and place the topology for the qkd network to simulate (in every node)
    orchestratorlib.install(config_file=yaml_config_file, inv_file=yaml_inv_file)
    
    # Once configured, lets run the qdts_node
